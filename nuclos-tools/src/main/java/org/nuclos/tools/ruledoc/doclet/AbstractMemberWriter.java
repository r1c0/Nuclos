...
public abstract class AbstractMemberWriter {
	 ...
	 * the generation.
	 */
	protected void printDeprecatedAPI(List<ProgramElementDoc> deprmembers, String headingKey) {
		if (deprmembers.size() > 0) {
			...
			for (int i = 0; i < deprmembers.size(); i++) {
				ProgramElementDoc member = deprmembers.get(i);
	 ...
	 * Print use info.
	 */
	protected void printUseInfo(List<ProgramElementDoc> members, String heading) {
		if (members == null) {
			return;
		}
		...
		if (members.size() > 0) {
			...
			writer.tableHeaderEnd();
			for (Iterator<ProgramElementDoc> it = members.iterator(); it.hasNext();) {
