...
public class ResultVoReportSource implements ReportSource {

	private final ResultVO result;
	private final ReportOutputVO.Format format;
	
	public ResultVoReportSource(ResultVO result, ReportOutputVO.Format format) {
		super();
		this.result = result;
		this.format = format;
	}
	...
	public NuclosFile getReport() throws NuclosReportException {
		try {
			return ReportDelegate.getInstance().prepareExport(result, format);
		}
