...
public class LiveSearchSettingsPanel extends JPanel {
	...
	public LiveSearchSettingsPanel() {
		...
		try {
        	...
        	Errors.getInstance().showExceptionDialog(this,
        		"Exception while reading the preferences.", e);
        }
        oldSavedSelected = new ArrayList<String>(savedSelected);
        oldSavedDeselected = new ArrayList<String>(savedDeselected);
        ...
        for(String s : allEntities)       // Remaining: either deselected or new -> select
	...
	private void savePrefs() {
		try {
			PreferencesUtils.putStringArray(
				ClientPreferences.getUserPreferences().node("livesearch"),
				"selected",
				toArray(selPanel.getJListSelectedObjects()));
			PreferencesUtils.putStringArray(
				ClientPreferences.getUserPreferences().node("livesearch"),
				"deselected",
				toArray(selPanel.getJListAvailableObjects()));
        }
        catch(PreferencesException e) {
        	Errors.getInstance().showExceptionDialog(this,
        		"Exception while storing the preferences.", e);
        }
	}
	
	public void restoreOriginalPrefs() {
		try {
			PreferencesUtils.putStringArray(
				ClientPreferences.getUserPreferences().node("livesearch"),
				"selected",
				oldSavedSelected.toArray(new String[oldSavedSelected.size()]));
			PreferencesUtils.putStringArray(
				ClientPreferences.getUserPreferences().node("livesearch"),
				"deselected",
				oldSavedDeselected.toArray(new String[oldSavedDeselected.size()]));
        }
        catch(PreferencesException e) {
        	Errors.getInstance().showExceptionDialog(this,
        		"Exception while storing the preferences.", e);
