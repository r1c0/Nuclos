...
public class JEditTextArea extends JComponent {
	...
	public JEditTextArea(TextAreaDefaults defaults) {
		...
		/* workaround for the scrollbar problem */
		//painter.setSize( painter.getPreferredSize());
		new ComponentHandler().componentResized(new ComponentEvent(painter, 101));
	}
	...
	public void updateScrollBars() {
		...
		if (horizontal != null && width != 0) {
			horizontal.setValues(-horizontalOffset, width, 0, getMaxLineWidth());
	...
	/**
	 * Get the width of the longest line in the document.
	 * @return the width of the longest line (in pixel)
	 */
	public int getMaxLineWidth() {
		final int nLines = getLineCount();
		if (nLines == 0) {
			// default value of 5 times the component width (for compatibility reasons)
			return painter.getWidth() * 5;
		}
		else {
			int nMaxWidth = 0;
			for (int nLine = 0; nLine < nLines; nLine++) {
				nMaxWidth = Math.max(nMaxWidth, getLineWidth(nLine));
			}
	...
	private int getLineWidth(int nLine) {
		int nMaxWidth;
		FontMetrics fm = painter.getFontMetrics();
		nMaxWidth = (int) fm.getStringBounds(getLineText(nLine), painter.getGraphics()).getWidth();
		return nMaxWidth;
	}
	...
	public final void setCaretPosition(int caret) {
		...
		if (infoPanel != null) {
			...
			final int col = getCaretPosition() - getLineStartOffset(line);
			infoPanel.setCaretPos(line + 1, col + 1);
		}
	}

	/**
	 * Attaches an info panel to the editor for display of caret position, overwrite mode etc.
	 *
	 * @param panel the info panel
	 */
	public void setInfoPanel(TextAreaInfoPanel panel) {
		infoPanel = panel;
	}
	...
	public final void setOverwriteEnabled(boolean overwrite) {
		...
		if (infoPanel != null) {
			infoPanel.setOverwrite(overwrite);
		}
	}
	...
	private TextAreaInfoPanel infoPanel = null;
