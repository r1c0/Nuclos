 ...
 *
 * @author Slava Pestov
 * @version $Id: InputHandler.java,v 1.2 2009-06-18 07:21:57 beckschulze Exp $
 * @see org.nuclos.client.gef.editor.syntax.DefaultInputHandler
 *
...
public abstract class InputHandler extends KeyAdapter {
	...
	public static final ActionListener NEXT_WINDOW = new next_window();
	public static final ActionListener PREVIOUS_WINDOW = new previous_window();
	// FIX ELISA-6552
...
//	}
	...
	public static class insert_break implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			String sNewLine = "\n";
			String sLine = textArea.getLineText(textArea.getCaretLine());
			int index = 0;
			while (sLine != null && sLine.length() > 0 && index < sLine.length() && sLine.charAt(index++) == '\t') {
				sNewLine += "\t";
			}
/*
			int line = 0;

			// Nice feature, but should be optimized before enabled...
			boolean bBracket = false;

			// If the first non-blank character to the left of the caret is a {
			int nFrom = sLine.lastIndexOf("{");
			if(nFrom != -1) {
				String substring = sLine.substring(nFrom + 1);
...
/*			if( bBracket == true) {
				textArea.setCaretPosition(textArea.getLineEndOffset(line + 1 ) -1);
			}
*/
		}
	}
	...
	public static class insert_tab implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (textArea.getSelectionStart() == textArea.getSelectionEnd()) {
				// When no text selected, just enter a tab
				textArea.overwriteSetSelectedText("\t");
			}
			else {
				// todo: else indent the selected text by one tab
			}
		}
	}
	...
	public static class insert_char implements ActionListener,
		...
		public void actionPerformed(ActionEvent evt) {
			...
			// boolean bInsertSpecialCombination = false;

			if (textArea.isEditable()) {
				/*
				// Nice featur, but should be optimized before enabling...
				if (str.equals("\"")) {
					str = "\"\"";
					bInsertSpecialCombination = true;
				}
				else if(str.equals("(")) {
					str = "()";
					bInsertSpecialCombination = true;
				...
				*/
				...
				if(bInsertSpecialCombination == true) {
	...
	// FIX ELISA-6851/ELISA-6971 created
	public static class next_window implements ActionListener {
		@Override
		public void actionPerformed(ActionEvent evt)
		{
			MainController.cmdCycleThroughWindows(true);
		}
	}
	
	// FIX ELISA-6851/ELISA-6971 created
	public static class previous_window implements ActionListener {
		@Override
		public void actionPerformed(ActionEvent evt)
		{
			MainController.cmdCycleThroughWindows(false);
		}
	}
}
