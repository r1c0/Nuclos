...
/**
 * Value object representing a complete process model with all its subprocesses and transitions.
 * <br>
...
public class ProcessMonitorGraphVO implements Serializable {
	...
	private Set<SubProcessVO> mpStates = new HashSet<SubProcessVO>();

	// Set for transitions between subprocesses
	private Set<ProcessTransitionVO> mpTransitions = new HashSet<ProcessTransitionVO>();

	public ProcessMonitorGraphVO() {
		this(null);
	}

	public ProcessMonitorGraphVO(ProcessMonitorVO statemodelvo) {
	 ...
	 */
	public ProcessMonitorVO getStateModel() {
	 ...
	 */
	public Set<SubProcessVO> getStates() {
	 ...
	 */
	public void setStates(Set<SubProcessVO> states) {
	 ...
	 */
	public Set<ProcessTransitionVO> getTransitions() {
	 ...
	 */
	public void setTransitions(Set<ProcessTransitionVO> transitions) {
		this.mpTransitions = transitions;
	}

	/**
	 * TODO validation is not active at the moment
	...
	public void validate() throws CommonValidationException {
		...
		if (!this.getStateModel().isRemoved()) {
			if (StringUtils.isNullOrEmpty(this.getStateModel().getName())) {
				throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.modelname"));
			}
			if (StringUtils.isNullOrEmpty(this.getStateModel().getDescription())) {
				throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.modeldescription"));
			}
	...
	private Map<Integer, String> validateStates() throws CommonValidationException {
		...
		for (SubProcessVO statevo : this.getStates()) {
			if (!statevo.isRemoved()) {		//ignore removed states
				if (StringUtils.isNullOrEmpty(statevo.getStatename())) {
					throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.statename"));
				}
				...
				if (result.containsKey(statevo.getId())) {
					throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.duplicateid"));
				}
				if (result.containsValue(statevo.getStatename())) {
					throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.duplicatestate"));
				}
				if (numerals.containsKey(statevo.getId())) {
					throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.duplicatenumerals",
						numerals.get(statevo.getId()).toString(),  statevo.getStatename()));
				}
				result.put(statevo.getId(), statevo.getStatename());
				numerals.put(statevo.getId(), statevo.getStatename());
			}
		}
		return result;
	}
	...
	private void validateTransitions(Map<Integer, String> mpStates) throws CommonValidationException {
		...
		for (ProcessTransitionVO statetransitionvo : this.getTransitions()) {
			...
			if (!statetransitionvo.isRemoved()) {
				if (statetransitionvo.getStateSource() == null) {
					...
					if (!statetransitionvo.isAutomatic()) {
						throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.invalidstarttransition"));
					}
				}
				else {
					if (!mpStates.containsKey(statetransitionvo.getStateSource())) {
						throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.invalidstart"));
					}
				}
				if (!mpStates.containsKey(statetransitionvo.getStateTarget())) {
					throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.invalidend"));
				}
				if (statetransitionvo.getStateTarget().equals(statetransitionvo.getStateSource())) {
					throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.startequalsend"));
				}
		...
		if (iStartTransitionCount == 0) {
			throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.nostartstate"));
		}
		if (iStartTransitionCount > 1) {
			throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.toomanystartstates"));
		}
	}
	 ...
	 */
	private void checkDuplicateTransition(ProcessTransitionVO statetransitionvo) throws CommonValidationException {
		for (ProcessTransitionVO statetransitionvo2 : this.getTransitions()) {
			if (!statetransitionvo2.isRemoved()) {
				if (!statetransitionvo.getClientId().equals(statetransitionvo2.getClientId())) {
					...
					{
						throw new CommonValidationException(StringUtils.getParameterizedExceptionMessage("statemachine.error.validation.graph.duplicatetransition"));
					}
				}
			}
		}
