...
/**
 * Structure (meta data) of a Lookup key field in a masterdata table.
 * <br>
 ...
 * @version 01.00.00
 */
public class CollectableMasterDataLookupKeyEntityField extends AbstractCollectableEntityField {

	private static final Logger LOG = Logger.getLogger(CollectableMasterDataLookupKeyEntityField.class);
	...
	/**
	 * creates a <code>CollectableEntityField</code> representing the Lookup key field <code>mdmetafield</code>.
	 * The collection of enumerated fields is built lazily.
	 * @param mdmetafield
	 */
	public CollectableMasterDataLookupKeyEntityField(MasterDataMetaFieldVO mdmetafield, String entityName) {
		this.mdmetafield = mdmetafield;
		this.entityName = entityName;
		final String sLookupEntity = mdmetafield.getLookupEntity();
		if (sLookupEntity == null) {
			throw new NuclosFatalException(StringUtils.getParameterizedExceptionMessage("clctmdfkef.missing.Lookupentity.error", mdmetafield.getFieldName()));
		}
	}
	...
	public String getReferencedEntityName() {
		final String result = this.mdmetafield.getLookupEntity();
		assert result != null;
		return result;
	}
	...
	public String getReferencedEntityFieldName() {
		...
		return LangUtils.defaultIfNull(this.mdmetafield.getLookupEntityField(), sFieldNameDefault);
	}
	...
	public String getDescription() {
		return SpringLocaleDelegate.getInstance().getTextFallback(mdmetafield.getResourceSIdForDescription(), this.mdmetafield.getDescription());
	}

	@Override
	public String getLabel() {
		return SpringLocaleDelegate.getInstance().getTextFallback(mdmetafield.getResourceSIdForLabel(), this.mdmetafield.getLabel());
	}
	...
	public CollectableField getDefault() {
		try {
			...
			else if(this.mdmetafield.getLookupEntity() == null || this.mdmetafield.getLookupEntityField() == null) {
				return super.getDefault();
			}
			else {
