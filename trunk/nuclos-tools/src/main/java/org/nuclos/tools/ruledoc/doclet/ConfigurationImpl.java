...
public class ConfigurationImpl extends Configuration {
	...
	protected String ALLE_REGELN = "Alle Regeln";
	...
	public static final boolean USE_STANDARD_SOURCE_WRITER = false;
	 ...
	 */
	public final MessageRetriever standardCommonMessage;
	...
	private ConfigurationImpl() {
		standardCommonMessage = new MessageRetriever(this,
	...
	public boolean validOptions(String options[][], DocErrorReporter reporter) {
		...
		for (int oi = 0; oi < options.length; ++oi) {
			...
			if (opt.equals("-helpfile")) {
				...
				if (!help.exists()) {
					reporter
							.printError(getText("doclet.File_not_found", os[1]));
					return false;
				}
				helpfile = true;
			}
	...
	/**
	 * Returns the given file-path with a trailing file-path-delimiter, such as
	 * "/", "\", system-dependent.
	 *
	 * @param filePath
	 *            The file-path, which should be returned with trailing path
	 *            delimiter. If filePath has a path-delimiter already, then
	 *            filePath is returned. If filePath is "", "" is returned.
	 * @return The file-path with trailing path-delimiter, or an empty String if
	 *         filePath is empty or null.
	 */
	public static String includeTrailingFilePathDelimiter(String filePath) {
