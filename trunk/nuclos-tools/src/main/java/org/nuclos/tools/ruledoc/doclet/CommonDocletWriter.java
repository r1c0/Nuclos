 ...
 * @author novabit
 */
public class CommonDocletWriter extends HtmlDocWriter {
	 ...
	 */
	public CommonDocletWriter(ConfigurationImpl configuration, String filename)
			throws IOException {
	 ...
	 */
	public CommonDocletWriter(ConfigurationImpl configuration, String path,
	...
	public String replaceDocRootDir(String htmlstr) {
		...
		while (true) {
			...
			if (relativepathNoSlash.length() > 0
					&& previndex < htmlstr.length()
	...
	public void printNoFramesTargetHyperLink(String link, String where,
			String target, String label, boolean bold) {
		script();
		println("  <!--");
	...
	private void printMethodInfo(MethodDoc method) {
		...
		if (intfacs.length > 0 || overriddenMethod != null) {
			...
			if (overriddenMethod != null) {
				MethodWriterImpl.printOverridden(this, method.overriddenType(),
						overriddenMethod);
			}
			printTagsInfoFooter();
			ddEnd();
	 ...
	 *            array.
	 * @param includeScript
	 *            boolean true if printing windowtitle script. False for files
	 *            that appear in the left-hand frames.
	 */
	public void printHtmlHeader(String title, String[] metakeywords,
		...
		if (!configuration.notimestamp) {
			print("<!-- Generated by javadoc (build "
					+ ConfigurationImpl.BUILD_DATE + ") on ");
			print(today());
			println(" -->");
		}
		...
		if (metakeywords != null) {
			for (int i = 0; i < metakeywords.length; i++) {
				println("<META NAME=\"keywords\" " + "CONTENT=\""
						+ metakeywords[i] + "\">");
			}
		}
		println("");
	 ...
	 *
	 * @param header
	 *            If true print navigation bar at the top of the page else print
	 *            the nevigation bar at the bottom.
	 */
	...
	protected void navShowLists(String link) {
		print(getHyperLink(link + "?" + path + filename, "", configuration
				.getText("doclet.FRAMES"), true, "", "", "_top"));
	}
	...
	protected void navLinkIndex() {
		...
		printHyperLink(
				relativePath
						+ (configuration.splitindex ? DirectoryManager
						.getPath("index-files")
						+ fileseparator : "")
						+ (configuration.splitindex ? "index-1.html"
						: "index-all.html"), "", configuration
	...
	protected void navLinkHelp() {
		...
		navCellStart();
		printHyperLink(relativePath + helpfilenm, "", "API Hilfe", true, "NavBarFont1");
		navCellEnd();
	}
	...
	/**
	 * Print Html tag for table elements. The tag printed is &lt;TD
	 ...
	 * @param isBold
	 *            true if the label should be bold.
	 */
	...
	public String getLink(LinkInfoImpl linkInfo) {
		...
		String link = ((LinkOutputImpl) factory.getLinkOutput(linkInfo))
				.toString();
	...
	public String getClassLink(ClassDoc cd, String where, String label,
		...
		if (nameUnspecified) {
			...
			String title = where = "Name der Regel";
			if (cd.isIncluded()) {
	...
	public com.sun.tools.doclets.internal.toolkit.util.MessageRetriever msg(
		...
		if (checkVersion && !configuration.printedVersion) {
			configuration.standardCommonMessage.notice("doclet.version",
					CommonDoclet.BUILD_DATE);
			configuration.printedVersion = true;
		}
		return configuration.standardCommonMessage;
	}
	 ...
	 * @param bold
	 *            true if the link should be bold.
	 * @param style
	...
	public String getCrossClassLink(String qualifiedClassName,
		...
		while ((periodIndex = packageName.lastIndexOf('.')) != -1) {
			className = packageName.substring(periodIndex + 1, packageName
					.length())
			...
			if (getCrossPackageLink(packageName) != null) {
				...
				return getHyperLink(configuration.extern.getExternalLink(
						packageName, relativePath, className + ".html"),
						refMemName == null ? "" : refMemName, label == null
						|| label.length() == 0 ? defaultLabel : label,
						bold, style, configuration.getText(
	...
	public void printQualifiedClassLink(int context, ClassDoc cd) {
		printLink(new LinkInfoImpl(context, cd, configuration.getClassName(cd),
				""));
	}

	/**
	 * Print Class link, with only class name as the link and prefixing plain
	 * package name.
	 */
	...
	public void printHtmlHeader(String title) {
		printHtmlHeader2(title, null, true);
	}

	public void printHtmlHeader(String title, String[] metakeywords) {
		printHtmlHeader2(title, metakeywords, true);
	}

	public void printHtmlHeader2(String title, String[] metakeywords,
			boolean includeScript) {
		println("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 "
				+ "Transitional//EN\" "
				+ "\"http://www.w3.org/TR/html4/loose.dtd\">");
	...
	public String getDocLink(int context, ClassDoc classDoc, MemberDoc doc,
		...
		if (!(doc.isIncluded() || Util.isLinkable(classDoc, configuration()))) {
			...
			return getLink(new LinkInfoImpl(context, classDoc, getAnchor(emd),
					label, bold));
		}
		else {
	...
	public String seeTagToString(SeeTag see) {
		...
		label = (label.length() > 0) ? ((isplaintext) ? label : getCode()
				+ label + getCodeEnd()) : "";
		...
		// not specified.
		String text = (isplaintext) ? seetext : getCode() + seetext
				+ getCodeEnd();
		...
		if (refClass == null) {
			...
			if (refPackage != null && refPackage.isIncluded()) {
				...
				if (packageCrossLink != null) {
					...
					// Class cross link found (possiblly to a member in the
					// class)
			...
			// refMemName is null.
			if (label.length() == 0) {
				label = (isplaintext) ? refClass.name() : getCode()
						+ refClass.name() + getCodeEnd();
			...
			text = (isplaintext) ? refMemName : getCode() + refMemName
					+ getCodeEnd();
	...
	private void printCommentTags(Doc doc, Tag[] tags, boolean depr,
			boolean first) {
		if (configuration.nocomment) {
			return;
		}
	 ...
	 * @param tags
	 *            array of text tags and inline tags (often alternating) present
	 *            in the text of interest for this doc
	...
	public String commentTagsToString(Tag holderTag, Doc doc, Tag[] tags,
		...
		for (int i = 0; i < tags.length; i++) {
			...
			if (tagelem instanceof SeeTag) {
				...
				// cases.
				String text = redirectRelativeLinks(tagelem.holder(), tagelem
						.text());
	...
	private String redirectRelativeLinks(Doc doc, String text) {
		...
		if (doc instanceof ClassDoc) {
			redirectPathFromRoot = DirectoryManager
					.getDirectoryPath(((ClassDoc) doc).containingPackage());
		}
		...
		if (begin >= 0) {
			...
			while (begin >= 0) {
				...
				if (begin == 0) {
					...
					configuration.getDocletSpecificMsg().warning(
							doc.position(), "doclet.malformed_html_link_tag",
							text);
					break;
				}
				if (end == -1) {
					...
					// malformed. The text
					// might be missing '>' character because the href has an
					// inline tag.
					break;
				}
				...
				begin = textBuff.toString().toLowerCase().indexOf("<a",
						begin + 1);
			}
	...
	/**
	 * According to the Java Language Specifications, all the outer classes and
	 * static nested classes are core classes.
	 */
			...
			if (pairs.length > 0) {
				...
				for (int j = 0; j < pairs.length; j++) {
					...
					for (Iterator<AnnotationValue> iter = annotationTypeValues.iterator(); iter
							.hasNext();) {
						annotation
								.append(annotationValueToString(iter
										.next()));
